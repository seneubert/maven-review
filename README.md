...
The analysis code repsoitory should be made available as soon as the project starts. **Ideally the maven reviewer can run the current state of the analysis by themselves as a preparation for the regular (biweekly) meetings with the analysis team.**
 
The whole working group is invited to contribute improvements to the analysis code. It is the decision of the analysis team whether or not to accept these contributions. A well tested workflow for this can be found [here](https://gitlab.cern.ch/help/workflow/forking_workflow.md).
